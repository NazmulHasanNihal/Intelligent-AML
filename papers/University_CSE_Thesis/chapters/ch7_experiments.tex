...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_temporal_split.pdf}
\caption{Zero-leakage 4-way chronological dataset partitioning timeline: 60\% Training ($\mathcal{D}_{\text{train}}$), 10\% Validation ($\mathcal{D}_{\text{val}}$), 10\% Calibration ($\mathcal{D}_{\text{cal}}$), and 20\% Out-of-Time Forward Testing ($\mathcal{D}_{\text{test}}$).}
\label{fig:ch7_temporal_split}
\end{figure}
...
\caption{Multi-criteria radar benchmark contrasting C-STGB against competitive tabular, spatial, and imbalanced baselines across PR-AUC, Macro F1, Recall, Imbalance Robustness, Camouflage Retention, and Banking SLA Compliance.}
\label{fig:ch7_radar_benchmark}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig1_pr_roc_curves.pdf}
\caption{Comparative Precision-Recall (PR) and Receiver Operating Characteristic (ROC) frontiers on Elliptic-v1, demonstrating C-STGB's superior area under curve under extreme class skew.}
\label{fig:ch7_pr_roc_curves}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_ablation_component_study.pdf}
\caption{Stepwise architectural ablation progression across Elliptic-v1, SAML-D, and IBM-AMLSim benchmarks, illustrating cumulative F1 improvements from baseline GCN ($10.33\%$) to full C-STGB ($91.42\%$).}
\label{fig:ch7_ablation_progression}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_leave_one_out_impact.pdf}
\caption{Leave-one-out component ablation impact on Elliptic-v1, showing marginal degradation in percentage points when individual architectural mechanisms are deactivated.}
\label{fig:ch7_leave_one_out}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig9_adversarial_camouflage_robustness.pdf}
\caption{Adversarial camouflage link robustness on Elliptic-v1 across increasing injected camouflage link ratios ($\rho \in [0.0, 0.80]$), demonstrating C-STGB's superior F1-score retention compared to baseline spatial and heterogeneous GNNs.}
\label{fig:ch7_camouflage_robustness}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig2_tsne_manifold_separation.pdf}
\caption{t-SNE latent manifold visualization: (a) Baseline GNN Layer 2 representation suffering from severe feature over-smoothing across high-degree hubs; and (b) C-STGB fused manifold exhibiting clear separation between illicit smurfing clusters and benign merchant traffic.}
\label{fig:ch7_tsne_separation}
\end{figure}
